...
export default function App() {
  ...
  const [groceryResult, setGroceryResult] = useState("");
  ...
  async function handleGrocery() {
    ...
    try {
      const resp = await fetch("/api/grocery", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ list: groceryList })
      });
      const data = await resp.json();
      setGroceryResult(data.result || data.error || "No result");
    } catch (err) {
      setGroceryResult("❌ Error: " + err.message);
    }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0c0e13", color: "#e8e4dc", fontFamily: "'DM Mono','Courier New',monospace", paddingBottom: 70 }}>
      ...
      <div style={{ padding: "12px 14px" }}>
        ...
        {tab === "grocery" && (
          <div style={{ maxWidth: 600, margin: "0 auto" }}>
            <div style={{ background: "#13151d", border: "1px solid #1e2130", borderRadius: 6, padding: 14, marginBottom: 16, fontSize: 12, color: "#555", lineHeight: 1.8 }}>
              Paste your weekly ordering list below. The app will instantly tell you which items to order from Restaurant Depot and which from Sysco — based on current prices.
            </div>
            <textarea
              value={groceryList}
              onChange={e => setGroceryList(e.target.value)}
              placeholder={"Paste your grocery/order list here...\n\nExample:\n- Chicken thighs\n- Yellow onions\n- Basmati rice\n- Heavy cream\n- Garlic"}
              style={{ width: "100%", height: 200, background: "#13151d", border: "1px solid #2a2d3a", color: "#e8e4dc", padding: 14, fontFamily: "'DM Mono',monospace", fontSize: 12, borderRadius: 6, resize: "vertical", lineHeight: 1.7 }}
            />
            <button
              onClick={handleGrocery}
              disabled={groceryLoading || !groceryList.trim()}
              style={{ width: "100%", marginTop: 10, background: groceryLoading ? "#333" : "#e85d2f", border: "none", color: "#fff", padding: "12px 0", cursor: groceryLoading ? "default" : "pointer", fontFamily: "'DM Mono',monospace", fontSize: 13, borderRadius: 6, letterSpacing: 1 }}>
              {groceryLoading ? "⏳ BREAKING DOWN YOUR LIST..." : "🛒 BREAK DOWN BY VENDOR"}
            </button>
            {groceryResult && (
              <div style={{ marginTop: 16, background: "#13151d", border: "1px solid #2a2d3a", borderRadius: 6, padding: 16 }}>
                <div style={{ fontSize: 9, color: "#555", letterSpacing: 2, marginBottom: 12 }}>VENDOR BREAKDOWN</div>
                <pre style={{ fontFamily: "'DM Mono',monospace", fontSize: 12, color: "#e8e4dc", whiteSpace: "pre-wrap", lineHeight: 1.8 }}>
                  {groceryResult}
                </pre>
                <button
                  onClick={() => { setGroceryList(""); setGroceryResult(""); }}
                  style={{ marginTop: 12, background: "none", border: "1px solid #2a2d3a", color: "#888", padding: "6px 14px", cursor: "pointer", fontFamily: "'DM Mono',monospace", fontSize: 11, borderRadius: 4 }}>
                  CLEAR
                </button>
              </div>
            )}
